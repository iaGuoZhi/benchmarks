...
csv_filename = f'{work_dir}/disk_lat.csv'
png_filename = f'{work_dir}/disk_lat.png'
...
for column in columns:
    df[column] = df[column].apply(lambda x: int(x) / 1000)
    ax1.plot(df['block_size'], df[column], label=column)

ax1.legend()
ax1.set_title(f'disk read latency vs block size')
ax1.set_xlabel('Size')
ax1.set_ylabel('us')
ax1.grid(True)
...
ax2.legend()
ax2.set_title(f'disk write latency vs block size')
ax2.set_xlabel('Size')
ax2.set_ylabel('us')
ax2.grid(True)
