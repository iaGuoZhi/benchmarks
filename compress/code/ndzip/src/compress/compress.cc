...
void compress_stream(const std::string &in, const std::string &out, const ndzip::extent &size,
    ...
    ex_time_ms elapse, elapse_kernel;
    double total_compress = 0;
    ...
    kernel_duration total_duration{};
    {
        ...
        while (auto *chunk = in_stream->read_exact()) {
            ...
            elapse_kernel = (t0_2 - t0_1);
            total_compress += elapse_kernel.count();
    ...
    std::string in_base = in.substr(in.find_last_of("/\\") + 1);
    in_base = in_base.substr(0, in_base.find_last_of("."));
    std::cerr << in_base << ":raw = " << n_chunks * in_file_size << " bytes";
    ...
    std::cerr << ", time = " << std::setprecision(5) << std::fixed
              << std::chrono::duration_cast<std::chrono::duration<double>>(total_duration).count() << "s, \t"
              << total_compress << " ms, \t" << elapse.count() <<" ms \n";
}
...
void decompress_stream(const std::string &in, const std::string &out, const ndzip::extent &size,
    ...
    for (;;) {
        ...
        auto t0_1 = std::chrono::high_resolution_clock::now();
        const auto compressed_length = offloader.decompress(chunk_buffer, chunk_buffer_length, output_buffer, size, &chunk_duration);
        auto t0_2 = std::chrono::high_resolution_clock::now();
        const auto compressed_size = compressed_length * sizeof(compressed_type);
        assert(compressed_length <= chunk_buffer_length);
        out_stream->commit_chunk(array_chunk_size);
        elapse_kernel = (t0_2 - t0_1);
        total_decompress += elapse_kernel.count();
        total_duration += chunk_duration;
    ...
    std::string in_base = in.substr(in.find_last_of("/\\") + 1);
    in_base = in_base.substr(0, in_base.find_last_of("."));
    std::cerr << in_base << ": decomp-time = " << std::setprecision(5) << std::fixed
              << std::chrono::duration_cast<std::chrono::duration<double>>(total_duration).count() << "s, \t"
              << total_decompress << " ms, \t" << elapse.count() <<" ms \n";
}
