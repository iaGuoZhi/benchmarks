...
pub fn run_buff_simd_compress64(test_file:&str, scl:usize, pred: f64) {
    let mut f = File::open(&test_file).expect("no file found");
    let num_bytes = f.metadata().unwrap().len() as usize;
    let mut input = BufReader::new(f);
    // let metadata = std::fs::metadata(&test_file).expect("unable to read metadata");
    // let mut buffer = vec![0; metadata.len() as usize];
    println!("f64 size ={}", num_bytes);
    ...
    loop{
        ...
        let fv = f64::from_le_bytes(buffer);
        if fv.is_nan(){
            float_vec.push(0.0);
        }else{
            float_vec.push(fv);
    ...
    // let mut seg = Segment::new(None,SystemTime::now(),0,file_vec.clone(),None,None);
    let mut seg = Segment::new(None,SystemTime::now(),0,float_vec.clone(),None,None);
    let org_size = seg.get_byte_size().unwrap();
    let comp = SplitBDDoubleCompress::new(10,10,scl);
    let start1 = Instant::now();
    let mut compressed= comp.buff_simd256_encode(&mut seg);
    ...
    let duration2 = start2.elapsed();
    // println!("Time elapsed in buff simd decompress function() is: {:?}", duration2);
    println!("Time: compress: {:?} , decompress : {:?}", duration1, duration2);

    println!("Performance:{}, {}, {}, {}, {}, {}", test_file, scl, pred,
             org_size as f64/ comp_size as f64,
             1000000000.0 * org_size as f64 / duration1.as_nanos() as f64 / 1024.0/1024.0/1024.0,
             1000000000.0 * org_size as f64 / duration2.as_nanos() as f64 / 1024.0/1024.0/1024.0
    )
}

pub fn run_buff_simd_compress32(test_file:&str, scl:usize, pred: f64) {
    let mut f = File::open(&test_file).expect("no file found");
    let num_bytes = f.metadata().unwrap().len() as usize;
    let mut input = BufReader::new(f);
    println!("f32 size ={}", num_bytes);
    let mut float_vec =  Vec::new();
    let mut nan_cnt =  0;
    loop{
        use std::io::ErrorKind;
        ...
        let fv = f32::from_le_bytes(buffer);
        if fv.is_nan(){
            nan_cnt +=1;
        }else{
            float_vec.push(fv);
    ...
    println!("nan-count={} vec-len={}", nan_cnt, float_vec.len());
...
pub fn run_buff_encoding_decoding(test_file:&str, scl:usize, pred: f64) {
    ...
    println!("Performance:{},{},{},{},{},{},{},{},{},{}", test_file, scl, pred,
             ...
             1000000000.0 * org_size as f64 / duration1.as_nanos() as f64 / 1024.0/1024.0/1024.0,
             1000000000.0 * org_size as f64 / duration2.as_nanos() as f64 / 1024.0/1024.0/1024.0,
             1000000000.0 * org_size as f64 / duration3.as_nanos() as f64 / 1024.0/1024.0/1024.0,
             1000000000.0 * org_size as f64 / duration4.as_nanos() as f64 / 1024.0/1024.0/1024.0,
             1000000000.0 * org_size as f64 / duration5.as_nanos() as f64 / 1024.0/1024.0/1024.0,
             1000000000.0 * org_size as f64 / duration6.as_nanos() as f64 / 1024.0/1024.0/1024.0
