...
static void Compress(int num, ull *inbuf, unsigned char **inter, int predsizem1, int threads, int chunksize, ull **fcm, ull **dfcm, double *elapse, size_t *outsize)
{
  ...
  *outsize += 4;
  ...
  for (i = 0; i < threads; i++) {
    ...
    *elapse += t3.tv_sec + t3.tv_usec / 1000000.0 - t2.tv_sec - t2.tv_usec / 1000000.0;
...
{
  int i, num, predsizelg2, threads, chunksize, chunkrepeat, predsizem1, val, cnt, in, BUFSIZE;
  ...
  if (5 == argc) {  // compress
    ...
    while (num > 0) {
      insize += num;
      Compress(num, input, inter, predsizem1, threads, chunksize, fcm, dfcm, &elapse, &outsize);
      totelapse += elapse;
      num = fread(input, 8, BUFSIZE >> 3, stdin);
    }
    ...
    fprintf(stderr, "cr= %.3f\t totc-time: %.2f ms\t comp-time: %.2f ms\n", 
            8.0 * insize / outsize, 1000.0 * ctime, 1000.0 * totelapse);
    ...
    fprintf(stderr, "dt= %.3f GB/s\t totd-time: %.2f\tdecomp-time: %.2f ms\n", 1000.0 * dtime, 1000.0 * totelapse);
  } else {
